...
@given(integers())
def test_safe_percent__100_percent_when_denomenator_is_zero(numerator):
...
def test_safe_percent__regular_fraction_otherwise(numerator, denomenator):
    assert _safe_percent(numerator, denomenator) == (
        Decimal(numerator) / Decimal(denomenator) * Decimal(100)
    ).quantize(Decimal("0.0001"))


def test_percent_branches_covered__given_invalid_object():
